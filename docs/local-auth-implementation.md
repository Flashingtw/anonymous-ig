...
新增 `migrations/0004_add_local_admin_auth.sql`；本次未修改 `0001`、`0002`。隔離候選不包含 `0003`。

`0004` 支援 GitHub-only、local-only、dual identity，至少一種完整 identity；重建 admins、admin_sessions、audit_logs 並保留既有 IDs、sessions、audit、投稿及 foreign keys，增加 username 唯一性、最後一位 enabled owner 防護 triggers、D1 limiter table。正式套用前必須確認備份與現有資料符合 constraints。

隔離候選的 shared fixtures、migration tests、workerd 與 Wrangler tests 一律只套 `0001＋0002＋0004`；不依賴 renderer schema。測試均為獨立資料庫，不是 production。
...
| UI | `frontend/admin/index.html`, `frontend/assets/admin.js`, `admin-auth.js`, `styles.css`, `frontend/content.json`；api.js 保留 main |
...
上述為 Local Auth 相關檔案。隔離候選的公開頁、投稿 repository、pending 審核保持 main；沒有 renderer、品牌或字型變更。
...
以下指令建立目前 checkout 的 8787 開發環境 moderator，使用預設 `.wrangler/state/`；其他 checkout 或隔離測試庫互不相通。登入名稱中的 local 指帳密身分；CLI 的 `--local` 才指定本機資料庫。
...
以下是來源 checkpoint `6a0f985` 的歷史驗證，不是隔離候選的測試數或 UI 簽核。候選的當次結果見 [隔離檢查](deployment-isolation.md)。
...
先備份並確認實際 migration 狀態；候選只包含 0001、0002、0004，不套 0003。若現場 ledger／schema 與預期不符，先停止並重做 migration review。先 schema，再程式與同部署的管理 UI；初次部署維持 local 關閉並驗證 GitHub OAuth，再綁定現有 owner 同一列。確認 CPU 預算後，才受控啟用 local 並驗證帳密登入。開關為 false 時不能完成帳密登入驗證。完整順序以 README「既有站更新順序」為準。
...
- **工作區**：macOS `._*` 已從 source checks、assets、migration 探索排除；不清理使用者既有 metadata。來源 binding 已 commit／push 為 6a0f985；候選狀態見部署快照。
