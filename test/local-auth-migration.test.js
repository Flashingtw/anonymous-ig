...
test("0004 preserves 0001-0003 admins, sessions, audit logs, submissions, and foreign keys", (t) => {
  ...
  const submissionId = database.prepare(`
  ...
  assert.deepEqual(plainRow(database.prepare(`
    SELECT content, status, render_status, rendered_image_key, rendered_at,
           template_id, renderer_version, render_version
    FROM submissions WHERE id = ?
  `).get(submissionId)), {
    content: "legacy submission",
    status: "approved",
    render_status: "ready",
    rendered_image_key: "renders/legacy.png",
